...
export type IsSetupQueryVariables = Exact<{ [key: string]: never }>;

export type IsSetupQuery = { __typename?: "Query"; setup: boolean };
...
export const IsSetupDocument = `
    query IsSetup {
  setup
}
    `;
export const useIsSetupQuery = <TData = IsSetupQuery, TError = ClientError>(
  client: GraphQLClient,
  variables?: IsSetupQueryVariables,
  options?: UseQueryOptions<IsSetupQuery, TError, TData>,
  headers?: RequestInit["headers"],
) =>
  useQuery<IsSetupQuery, TError, TData>(
    variables === undefined ? ["IsSetup"] : ["IsSetup", variables],
    fetcher<IsSetupQuery, IsSetupQueryVariables>(client, IsSetupDocument, variables, headers),
    options,
  );

useIsSetupQuery.getKey = (variables?: IsSetupQueryVariables) =>
  variables === undefined ? ["IsSetup"] : ["IsSetup", variables];
export const SetupDocument = `
  ...
  setup(input: $input)
    ...
    `;
export const useSetupMutation = <TError = ClientError, TContext = unknown>(
  client: GraphQLClient,
  options?: UseMutationOptions<SetupMutation, TError, SetupMutationVariables, TContext>,
  headers?: RequestInit["headers"],
) =>
  useMutation<SetupMutation, TError, SetupMutationVariables, TContext>(
    ["Setup"],
    (variables?: SetupMutationVariables) =>
      fetcher<SetupMutation, SetupMutationVariables>(client, SetupDocument, variables, headers)(),
    options,
  );
