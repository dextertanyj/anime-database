export const setCompare = <T>(
	...
	const lhsOnly: Set<T> = new Set(
		[...lhs].filter(
			(item) => !Array.from(rhs).some((element) => comparator(item, element)),
		),
	);
	const intersect: Set<T> = new Set(
		[...lhs].filter((item) =>
			Array.from(rhs).some((element) => comparator(item, element)),
		),
	);
	const rhsOnly: Set<T> = new Set(
		[...rhs].filter(
			(item) => !Array.from(lhs).some((element) => comparator(element, item)),
		),
	);
